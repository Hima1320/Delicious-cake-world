...
from mayflower.models import catdb,cakedb,cntdb


def homepage(request):
    data=catdb.objects.all()
    product = cakedb.objects.all()
    context = {
        'product': product,
        'data': data,
    }
...
def discategory(request,itemcatg):
    ...
    catg=itemcatg.upper()
    products=cakedb.objects.filter(Category=itemcatg)
    context={
        'products':products,
        'catg':catg,
        'data':data
...
def cakedetails(request,dataid):
    data=cakedb.objects.get(id=dataid)
    return render(request,"cakedetails.html",{'dat':data})
