...
from mayflower.models import admindb,catdb,cakedb,cntdb
...
def cakepage(request):
    ...
    return render(request,"addcake.html",{'dat':dat})


def cakesave(request):
    if request.method == "POST":
    ...
    obj = cakedb(Cakename=nam,Price=pri,About=ab,Image = img,Category=ca)
    obj.save()
    return redirect(cakepage)

def cakedisplay(request):
    data=cakedb.objects.all()
    return render(request,"displaycake.html",{'data':data})


def cakeedit(request, dataid):
    dat=cakedb.objects.get(id=dataid)
    da=catdb.objects.all()
    print(dat)
    return render(request, "editcake.html", {'dat':dat,'da':da})

def cakeupdate(request, dataid):
    if request.method == "POST":
        nam = request.POST.get('bname')
        pri = request.POST.get('price')
        ab = request.POST.get('abt')
        ...
        try:
            ...
            file = cakedb.objects.get(id=dataid).Image
        cakedb.objects.filter(id=dataid).update(Icecreamname=nam,Price=pri,About=ab,Image = file,Category=ca)
        return redirect(cakedisplay)

def cakedelete(request, dataid):
    dat=cakedb.objects.filter(id=dataid)
    dat.delete()
    return redirect(cakedisplay)
